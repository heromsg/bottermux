...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            ...
            + 'TEL;type=CELL;type=VOICE;waid=5511932069468:+55 (11) 93206-9468\n' // WhatsApp ID + phone number
            + 'END:VCARD'
prefix = '.'
blocked = []
			...
			mess = {
				...
				only: {
					...
					Badmin: '[❗] Este comando só pode ser usado quando o bot se torna um administrador! ❌',
                                        daftarB: `──「 lista 」──\n Olá cocô!\nVocê ainda não está listado no banco de dados., \n\nCommand : ${prefix}lista de nomes|idade\n Exemplo : ${prefix}Lista ryz|17`,
				}
			}
    			const apakah = ['Sim','não']
        		const bisakah = ['não','não']
		        const kapankah = ['mais dias','mais uma semana','Meses de Distância','Mais um ano']
			const botNumber = client.user.jid
			const ownerNumber = ["5511932069468@s.whatsapp.net"] // replace this with your number
				...
				case 'demote':
					...
					if (mentioned.length > 1) {
						...
						for (let _ of mentioned) {
							teks += `comando recebido, grupo admin :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					} else {
						mentions(`comando recebido, Perdeu @${mentioned[0].split('@')[0]}\n o cargo de administrativo _*${groupMetadata.subject}*_`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
                                case 'hentai':
                  ...
                  case 'promote':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Marque o monkey que você deseja promover!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Comando recebido, adicionando posição como administrador :\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
						...
						mentions(` Comando recebido, adicionada a posição como administrador: @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break
				  case 'wa.me':
  ...
  client.updatePresence(from, Presence.composing) 
    ...
    client.sendMessage(from, options, text, { quoted: mek } )
			...
			case 'quotes':
              ...
              case 'owner':
                case 'creator':
                  client.sendMessage(from, {displayname: "𝓜𝓪𝓽𝓱𝓮𝓾𝓼T. T", vcard: vcard}, MessageType.contact, { quoted: mek})
               client.sendMessage(from, 'Aqui está o número do meu dono, salve-o mais tarde caso o bot caia(wa.me/5511932069468',MessageType.text, { quoted: mek} )
                break
	case 'hidetag':
                client.updatePresence(from, Presence.composing) 
                if (!isUser) return reply(mess.only.daftarB)
              ...
              await client.sendMessage(from, options, text)
               break
                                case 'tiktokstalk':
					try {
						if (args.length < 1) return client.sendMessage(from, 'Qual é o nome de usuário? ', text, {quoted: mek})
                                                if (!isUser) return reply(mess.only.daftarB)
				...
				case 'letrananeve':
					...
					var gbl7 = gh.split("|")[0];
					var gbl8 = gh.split("|")[1];
					if (args.length < 1) return reply(`Enviar comando ${prefix}letrananeve texto1|texto2, Exemplo ${prefix}letrananeve bom|dia`)
                                        if (!isUser) return reply(mess.only.daftarB)
					reply(mess.wait)
				...
				case 'marvellogo':
					...
					var gbl5 = gh.split("|")[0];
					var gbl6 = gh.split("|")[1];
					if (args.length < 1) return reply(`enviar comando ${prefix}marvellogo texto1|texto2, exemplo ${prefix}marvellogo vingadores|avante`)
                                        if (!isUser) return reply(mess.only.daftarB)
					reply(mess.wait)
		...
		case 'info':
               client.updatePresence(from, Presence.composing) 
                 if (!isUser) return reply(mess.only.daftarB)
                 if (args.length < 1) return reply(`Digite número\nexemplo : ${prefix}info 5511932069468`)
		...
		case 'spamcall':
               client.updatePresence(from, Presence.composing)
                 if (!isUser) return reply(mess.only.daftarB)
                 if (args.length < 1) return reply(`digite o Numero\n exemplo : ${prefix}spamcall 994405374134`)
				...
				case 'wikien':
					...
					break				
				case 'ytmp3':
					if (args.length < 1) return reply('Qual url?')
					...
					break
					case 'ytmp4':
				if (args.length < 1) return reply('qual link do youtube?')
					tels = body.slice(7)				
					reply(mess.wait)
				...
				case 'testime':
					...
					setTimeout( () => {
					client.sendMessage(from, 'Faltam 5 segundos.', text) // ur cods
					}, 5000) // 1000 = 1s,
					setTimeout( () => {
					client.sendMessage(from, 'Faltam 10 segundos.', text) // ur cods
					}, 0) // 1000 = 1s,
					break
				case 'semoji':
				...
				case 'nulis': 
				case 'tulis': ini
					if (args.length < 1) return reply('O que eu te disse para escrever, irmão?')
                                        if (!isUser) return reply(mess.only.daftarB)
					teks = body.slice(7)
					reply(mess.wait)
				...
				case 'tts':
				   ...
				   if (args.length < 1) return client.sendMessage(from, 'Qual código de linguagem é?', text, {quoted: mek})
                                   if (!isUser) return reply(mess.only.daftarB)
					const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return client.sendMessage(from, 'Texto é que', text, {quoted: mek})
					dtt = body.slice(8)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 600
					? reply('Texto é principalmente')
				...
				case 'adminlist':
					client.updatePresence(from, Presence.composing) 
                                        if (!isUser) return reply(mess.only.daftarB)
					if (!isGroup) return reply(mess.only.group)
					teks = `Lista do admin do grupo *${groupMetadata.subject}*\nTotal : ${groupAdmins.length}\n\n`
					no = 0
				...
				case 'block':
					...
					client.sendMessage(from, `comando recebido, bloqueio ${body.slice(8)}@c.us`, text)
					break
				case 'hilih':
					client.updatePresence(from, Presence.composing) 
				...
				case 'setppbot':
				client.updatePresence(from, Presence.composing) 
				if (!isQuotedImage) return reply(`Envie uma foto com legendas ${prefix}setppbot ou uma etiqueta de imagem que foi enviada`)
					if (!isOwner) return reply(mess.only.ownerB)
				...
				case 'Acorde':
					if (args.length < 1) return reply('o título da canção onde')
                                        if (!isUser) return reply(mess.only.daftarB)
					tels = body.slice(7)					
					...
					break
				case 'letra':
					if (args.length < 1) return reply('Qual música você quer a letra?')
                                        if (!isUser) return reply(mess.only.daftarB)
					tels = body.slice(7)
			...
			case 'igstalk':
               ...
               client.updatePresence(from, Presence.composing) 
              options = {
          text: `Este Grupo é de propriedade : @${from.split("-")[0]}`,
          contextInfo: { mentionedJid: [from] }
           ...
           break
				...
				case 'add':
					...
					if (args[0].startsWith('08')) return reply('Use código de país mais o numero')
					try {
						num = `${args[0].replace(/ /g, '')}@s.whatsapp.net`
						client.groupAdd(from, [num])
					} catch (e) {
						console.log('Error :', e)
						reply('Falhou em adicionar alvo, possivelmente porque é privado')
					}
					break
				...
				case 'kick':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('marque quem vai tomar o ban')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Comandos recebidos, removendo :\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
						...
						mentions(`Comandos recebidos, removendo: @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					client.sendMessage(mentioned, 'tomou bankk😂', text)
					}
					break
				case 'exe':
	              client.updatePresence(from, Presence.composing) 
				...
				case 'bugreport':
				client.updatePresence(from, Presence.composing) 
				if (args.length < 1) return reply('bugou oque?')
					tek = body.slice(10)
					bug = {
					...
					client.sendMessage(from, 'Seu relatório foi enviado ao proprietário do BOT, o relatório falso não será respondido.', text, {quoted: mek})
					break
               case 'Faça':
               client.updatePresence(from, Presence.composing) 
	    ...
	    case 'kapankah':
			...
			case 'closegc':
				...
				case 'stikergif':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.addOutputOptions([`-vcodec`,`libwebp`,`-vf`,`scale='min(520,iw)':min'(520,ih)':force_original_aspect_ratio=decrease,fps=20, pad=320:320:-1:-1:color=white@0.0, split [a][b]; [a] palettegen=reserve_transparent=on:transparency_color=ffffff [p]; [b][p] paletteuse`])
							.toFormat('webp')
							.save(ran)
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								reply(`❌ Falhou, na conversão ${tipe}  para adesivo`)
							})
							.on('end', function () {
				...
				case 'toimg':
				    client.updatePresence(from, Presence.composing)
                                    if (!isUser) return reply(mess.only.daftarB)
					if (!isQuotedSticker) return reply(' ❌ responder adesivo')
					reply(mess.wait)
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('❌Falhou, ao converter adesivo em imagem  ❌')
                ...
                case 'ninjalogo':
                      if (args.length < 1) return reply(' Que texto?')
                      if (!isUser) return reply(mess.only.daftarB)
                      gh = body.slice(11)
                      gl1 = gh.split("|")[0];
                      gl2 = gh.split("|")[1];
               ...
               if (!isUser) return reply(mess.only.daftarB)
               if (anu.error) return reply(anu.error)
                 infomp3 = `* Canções encontradas!!!*\nJudul : ${anu.result.title}\nDuração : ${anu.result.duration}\nTamanho : ${anu.result.size}\n\n*ESPERE UM MINUTO ENVIADO POR FAVOR NÃO FAZ SPAM PUTA SEQUELADA*`
                ...
                break
                   ...
                   break
					...
					if (args.length < 1) return reply(`Parâmetro errado\nComando : ${prefix}cadastro nome|idade\nexemplo : ${prefix}cadastro matheus|15`)
					var reg = body.slice(8)
					var jeneng = reg.split("|")[0];
					var umure = reg.split("|")[1];
						user.push(sender)
						fs.writeFileSync('./src/user.json', JSON.stringify(user))Para usar bots
						client.sendMessage(from, `\`\`\`Registro bem-sucedido Com SN: TM08GK8PPHBSJDH10J\`\`\`\n\n\`\`\`Em ${date} ${time}\`\`\`\n\`\`\`[Nome]: ${jeneng}\`\`\`\n\`\`\`[Numero]: wa.me/${sender.split("@")[0]}\`\`\`\n\`\`\`[idade]: ${umure}\`\`\`\n\`\`\`Para usar o bot\`\`\`\n\`\`\Por favor`\`\`\`\n\`\`\`mande ${prefix}help\`\`\`\n\`\`\`\nTotal de Usuários ${user.length}\`\`\``, text, {quoted: mek})
					break
                                case 'welcome':
					if (!isGroup) return reply(mess.only.group)
                                        if (!isUser) return reply(mess.only.daftarB)
					if (!isGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('mande 1 para ativar')
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('recurso já está ativo')
						welkom.push(from)
						fs.writeFileSync('./src/welkom.json', JSON.stringify(welkom))
						reply('❬ SUCCSESS ❭ ao habilitar o recurso de boas-vindas neste grupo ')
					} else if (Number(args[0]) === 0) {
						welkom.splice(from, disable)
						fs.writeFileSync('./src/welkom.json', JSON.stringify(welkom))
						reply('❬ SUCCSESS ❭ ao desativar o recurso de boas-vindas neste grupo')
					} else {
						reply('mande 1 para ativar, 0 para desativar o recurso')
					}
                                        break
                                case 'fakta':
					fakta = body.slice(1)
					...
					if (args.length < 1) return reply(mess.blank)
                                        if (!isUser) return reply(mess.only.daftarB)
					tels = body.slice(7)
					if (tels.length > 15) return reply('O texto é longo, até 20 caracteres de comprimento')
					reply(mess.wait)
				...
				case 'txtfogo':
					if (args.length < 1) return reply(mess.blank)
                                        if (!isUser) return reply(mess.only.daftarB)
					tels = body.slice(7)
					if (tels.ength > 10) return reply('o texton é muito logo maximo 9 caracteres')
					reply(mess.wait)
